...
class SkillCreateOutput {
  ...
  header() {
    ...
    console.log(chalk.bold(chalk.magenta('\u2551')) + chalk.bold('  SCC Skill Creator                                              ') + chalk.bold(chalk.magenta('\u2551')));
...
`));
  ...
  footer() {
    ...
    console.log(chalk.dim('  Powered by Salesforce Claude Code'));
    console.log('\n');
  }
}
...
async function demo() {
  const output = new SkillCreateOutput('my-sf-project');
  ...
  output.patterns([
    ...
    },
    {
      ...
      trigger: 'when creating Apex service classes',
      confidence: 0.90,
      evidence: 'Observed in AccountService, ContactService, OpportunityService',
    },
    {
      name: 'Trigger Framework',
      trigger: 'when adding Apex triggers',
      confidence: 0.85,
      evidence: 'All triggers delegate to handler classes',
    },
    {
      name: 'TDD with Apex Tests',
      trigger: 'when adding features',
      confidence: 0.75,
      evidence: '95% test coverage, test classes created before implementation',
    },
  ]);

  output.instincts([
    { name: 'sf-conventional-commits', confidence: 0.85 },
    { name: 'sf-fflib-service-pattern', confidence: 0.90 },
    { name: 'sf-trigger-handler-pattern', confidence: 0.85 },
    { name: 'sf-tdd-workflow', confidence: 0.90 },
    { name: 'sf-package-manager', confidence: 0.95 },
    { name: 'sf-governor-limit-caution', confidence: 0.90 },
  ]);

  output.output(
    '.claude/skills/sf-patterns/SKILL.md',
    '.claude/homunculus/instincts/inherited/sf-instincts.yaml'
  );
