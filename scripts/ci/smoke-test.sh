...
if [ -f ".claude/hooks/scripts/governor-check.js" ]; then pass "hook scripts installed"; else fail "hook scripts installed"; fi
...
if [ ! -f ".claude/agents/sf-agentforce-agent.md" ]; then pass "no extended agent (correct)"; else fail "extended agent leaked into apex profile"; fi
...
# Platform agents
for f in deep-researcher.md sf-integration-agent.md; do
...
if [ ! -f ".claude/agents/sf-apex-agent.md" ]; then pass "no apex agent in lwc (correct)"; else fail "apex agent leaked into lwc profile"; fi
