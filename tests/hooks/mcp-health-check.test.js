...
function createTempDir() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'scc-mcp-health-'));
}
  ...
  if (await asyncTest('marks healthy command MCP servers and allows the tool call', async () => {
    ...
    try {
      ...
      const result = runHook(input, {
        CLAUDE_HOOK_EVENT_NAME: 'PreToolUse',
        SCC_MCP_CONFIG_PATH: configPath,
        SCC_MCP_HEALTH_STATE_PATH: statePath,
        SCC_MCP_HEALTH_TIMEOUT_MS: '100'
      });
    ...
    try {
      ...
      const result = runHook(
        ...
        },
        {
          ...
          SCC_MCP_CONFIG_PATH: configPath,
          SCC_MCP_HEALTH_STATE_PATH: statePath,
          SCC_MCP_RECONNECT_COMMAND: `node ${JSON.stringify(reconnectScript)}`,
          SCC_MCP_HEALTH_TIMEOUT_MS: '100'
        }
      );
